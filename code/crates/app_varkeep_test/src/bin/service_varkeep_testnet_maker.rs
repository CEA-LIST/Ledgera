...
fn main() {
    let (number_of_nodes, number_of_varkeep_clients) = {
        ...
        let number_of_varkeep_clients: u32 = args[2].parse().unwrap();
        (number_of_nodes, number_of_varkeep_clients)
    };

    {
        let path = Path::new("./service_varkeep_testnet/");
        if path.exists() {
    ...
    for node_id in 0..number_of_nodes {
        ...
        if node_id < number_of_varkeep_clients {
    ...
    for (node_index, private_key) in private_keys.into_iter().enumerate() {
        {
            let file_path = format!("./service_varkeep_testnet/node{}", node_index + 1);
        ...
        }
        {
            ...
            let mut file = File::create(format!(
                "./service_varkeep_testnet/node{}/private_key.txt",
                node_index + 1
            ))
            .unwrap();
        ...
        {
            let mut file = File::create(format!(
                "./service_varkeep_testnet/node{}/known_participants.txt",
                node_index + 1
            ))
            .unwrap();
        ...
        {
            let mut file = File::create(format!(
                "./service_varkeep_testnet/node{}/service_clients.txt",
                node_index + 1
            ))
            .unwrap();
            file.write_all(service_clients_file_content.as_bytes())
                .unwrap();
        }
    }
}
