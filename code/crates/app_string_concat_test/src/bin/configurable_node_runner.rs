...
use ledgera_app_string_concat::lat_binding::StrConcatBackend;
use ledgera_app_string_concat_tui::parser::StrConcatComputationParser;
use ledgera_app_string_concat_tui::printer::StrConcatComputationPrinter;
...
use ledgera_util_deployment::deployment::{read_known_participants, read_private_key};
use log::LevelFilter;
...
use ledgera_util_basic_tui::setup::{ledgera_tui_log_setup, ledgera_tui_setup};
...
async fn main() {
    let (node_name, roles, pki_folder_path) = {
        ...
        if args.len() != 4 {
            eprintln!("Usage: {} <node name> <roles abbrs. ('c' client, 's' storage, 'v' voter, 'l' log)> <pki folder path>", args[0]);
            std::process::exit(1);
        }
    ...
    let service = Arc::new(StrConcatBackend {});
    ...
    let mut ledgera_node_state: LedgerNodeState<
        ...
        ZenohBackend,
        StrConcatBackend,
    > = LedgerNodeState::new(comm_param, service, comm_api_ref, roles)
        .await
        .unwrap();
    ...
    match ledgera_node_state.run().await {
        ...
        Some(client_runtime) => {
            ledgera_tui_setup::<
                ...
                ZenohBackend,
                StrConcatBackend,
                StrConcatComputationParser,
                StrConcatComputationPrinter,
            >(node_name, client_runtime)
            .await;
        }
    }
}
